...
package org.wso2.carbon.identity.entitlement.xacml.core.cache;
...
public class EntitlementBaseCache<K extends IdentityCacheKey, V extends Serializable> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(EntitlementBaseCache.class);
    ...
    public EntitlementBaseCache(String cacheName, int timeout) {
        ...
        logger.debug("Cache : " + Entitlement_CACHE_NAME + "  is initialized");
    }
            ...
            if (cacheBuilder == null) {
                synchronized (Entitlement_CACHE_NAME.intern()) {
                    if (cacheBuilder == null) {
                        ...
                        logger.debug("Cache : " + Entitlement_CACHE_NAME + "  is built with time out value "
                                + ": " + cacheTimeout);
                    }
                }
            } else {
    ...
    public void updateToCache(K key, V entry) {
        ...
        logger.debug("Cache : " + Entitlement_CACHE_NAME + "  is populated with new entry ");
    ...
    public V getValueFromCache(K key) {
        ...
        if (cache != null) {
            if (cache.containsKey(key)) {
                logger.debug("Cache : " + Entitlement_CACHE_NAME + "  is HIT ");
                return cache.get(key);
            }
        }
        logger.debug("Cache : " + Entitlement_CACHE_NAME + "  is MISSED ");
        return null;
    }
    ...
    public void clearCacheEntry(K key) {
        ...
        if (cache != null) {
            if (cache.containsKey(key)) {
                cache.remove(key);
                logger.debug("Cache : " + Entitlement_CACHE_NAME + " entry is removed ");
            }
        }
    }
    ...
    public void clear() {
        ...
        if (cache != null) {
            try {
                cache.removeAll();
                logger.debug("Cache : " + Entitlement_CACHE_NAME + " is cleared");
            } catch (Exception e) {
